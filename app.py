...
def api_agenda_eventos():
    ...
    if request.method == "POST":
        ...
        # FIX COD-02: usar AGENDA_TIPO_COR global em vez de dict local
        cor = AGENDA_TIPO_COR.get(tipo, "#64748b")
        if not titulo or not data_inicio:
            return jsonify({"erro": "TÃ­tulo e data sÃ£o obrigatÃ³rios"}), 400
        conn = get_db()
        cur  = conn.execute("""INSERT INTO agenda_eventos
            ...
            (titulo, descricao, tipo, data_inicio, data_fim, dia_inteiro, lembrete_min,
             cor, u["nome"] if u else "admin", datetime.now().isoformat()))
    ...
    # â”€â”€ 1. Eventos manuais â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    rows = q("""SELECT * FROM agenda_eventos
                WHERE date(data_inicio) <= date(?) AND (data_fim IS NULL OR date(data_fim) >= date(?))
                ORDER BY data_inicio""", (fim, inicio))
    for r in rows:
        eventos.append({
            ...
            "color":       r["cor"] or AGENDA_TIPO_COR.get(r["tipo"], "#64748b"),
            "extendedProps": {
                "tipo":         r["tipo"],
                "tipoLabel":    AGENDA_TIPO_LABEL.get(r["tipo"], "Outro"),
    ...
    # â”€â”€ 2. Eventos automÃ¡ticos: resumo por dia de visitas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    auto_rows = q("""
        SELECT
            v.data,
            v.tipo,
    ...
    for r in auto_rows:
        ...
        desc_partes = []
        if r["normais"]:     desc_partes.append(f"Normais: {r['normais']}")
        if r["fechados"]:    desc_partes.append(f"Fechados: {r['fechados']}")
        if r["recuperados"]: desc_partes.append(f"Recuperados: {r['recuperados']}")
        if r["recusados"]:   desc_partes.append(f"Recusas: {r['recusados']}")
        ...
        eventos.append({
            ...
            "end":     r["data"],
            "allDay":  True,
            "color":   TIPO_CORES.get(tipo, "#64748b") + "cc",
            "display": "block",
            "extendedProps": {
                "tipo":        tipo,
                "tipoLabel":   TIPO_LABELS.get(tipo, tipo),
...
@app.route("/api/agenda/eventos/<int:id_evento>", methods=["PUT", "DELETE"])
@login_required
@nivel_min("admin")
def api_agenda_evento(id_evento):
    ...
    if not titulo or not data_inicio:
        return jsonify({"erro": "TÃ­tulo e data sÃ£o obrigatÃ³rios"}), 400
    conn = get_db()
    conn.execute("""UPDATE agenda_eventos SET titulo=?, descricao=?, tipo=?, data_inicio=?,
...
@login_required
def api_agenda_lembretes():
    """Retorna eventos manuais nas prÃ³ximas 24h para notificaÃ§Ãµes do browser."""
    from datetime import datetime as _dt, timedelta as _td
    agora   = _dt.now()
    limite  = agora + _td(hours=24)
    rows = q("""SELECT id_evento, titulo, tipo, data_inicio, dia_inteiro, lembrete_min
                FROM agenda_eventos
