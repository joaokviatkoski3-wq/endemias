...
def api_visita_detalhe(id_visita):
    try:
        return jsonify(visitas_core.detalhar(bh.db_target(), id_visita))
...
def api_visita_editar(id_visita):
    ...
    try:
        resultado = visitas_core.editar(bh.db_target(), id_visita, dados)
        ...
        return jsonify({"erro": str(exc)}), 404
    except (visitas_core.VisitaInvalida, visitas_core.ColetaComResultado) as exc:
